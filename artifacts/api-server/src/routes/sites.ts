...
// ---------------------------------------------------------------------------
// Site quality analyzer — no external service, pure file parsing
// ---------------------------------------------------------------------------
...
function analyzeSiteFiles(files: Record<string, string>) {
  const html = (
...
router.get("/sites/:id/analyze", requireAuth, async (req, res) => {
  const [site] = await db
    .select().from(sitesTable)
    .where(and(eq(sitesTable.id, String(req.params.id)), eq(sitesTable.userId, req.user!.id)))
    .limit(1);
  ...
  if (!files || Object.keys(files).length === 0) {
    res.status(409).json({ error: "no_files", message: "Site has no files yet — build it first." });
    return;
  }
  res.json(analyzeSiteFiles(files));
});
