...
⚡⚡⚡ ABSOLUTE RULE — BROWSER-ONLY OUTPUT ⚡⚡⚡
You MUST generate ONLY: HTML (.html), CSS (.css), and vanilla JavaScript (.js) files.
NEVER output ANY of these — they will be stripped and ignored:
  ✗ Python (.py), Ruby (.rb), PHP, Java, Go, Rust, TypeScript (.ts), SQL, Bash (.sh)
  ✗ Dockerfile, docker-compose.yml, .gitignore, .env, requirements.txt, package.json
  ✗ Any file inside backend/, server/, api/, alembic/, migrations/, node_modules/
  ✗ Any server-side framework (Flask, FastAPI, Express, Rails, Django, Laravel, Spring)
For "auth", "database", "API", or "backend" features:
  ✓ localStorage / sessionStorage for data persistence
  ✓ fetch() to public REST APIs (JSONPlaceholder, Open APIs) for demo data
  ✓ JavaScript setTimeout/Promise to simulate async operations
  ✓ Pure JS state machines for multi-step flows
Everything MUST run 100% in a browser tab — no server, no runtime, no compilation.
...
⚡⚡⚡ ABSOLUTE RULE — BROWSER-ONLY OUTPUT ⚡⚡⚡
Only output files with extensions: .html  .css  .js
NEVER output: Python, Dockerfile, backend/, requirements.txt, alembic/, .py, .rb, .sql, .sh, package.json, .gitignore, .env, or any server-side file.
For "auth/database/API" features: use localStorage, sessionStorage, JS fetch to public APIs, or simulate with JavaScript. ALL code must run in a browser tab with zero server-side dependencies.
...
function sanitizeFiles(raw: Record<string, unknown> | undefined, plan: SitePlan): SiteFiles {
  ...
  // STATIC WEB ONLY — strip server-side and backend files so Python/Docker
  // artifacts generated by the LLM never reach the user's site output.
  const ALLOWED_EXTS = new Set([".html", ".css", ".js", ".json", ".svg", ".ico", ".txt", ".xml", ".webmanifest"]);
  const BLOCKED_DIR_PREFIXES = [
    "backend/", "server/", "api/", "node_modules/", ".git/",
    "venv/", "__pycache__/", "alembic/", "migrations/", "dist/", "build/",
  ];
  const BLOCKED_FILENAMES = new Set([
    "dockerfile", "docker-compose.yml", "docker-compose.yaml",
    ".gitignore", ".env", ".env.example", ".env.local",
    "requirements.txt", "package.json", "package-lock.json",
    "tsconfig.json", "makefile", "gemfile", "cargo.toml",
    "go.mod", "go.sum", "pom.xml", "build.gradle", "setup.py",
  ]);

  for (const [k, v] of Object.entries(raw)) {
    if (typeof v !== "string") continue;
    const origPath = k.replace(/^\/+/, "");
    const pathLower = origPath.toLowerCase();
    if (pathLower.length === 0 || pathLower.length > 200 || pathLower.includes("..")) continue;

    // Skip blocked directory prefixes
    if (BLOCKED_DIR_PREFIXES.some(prefix => pathLower.startsWith(prefix))) continue;
    ...
    if (BLOCKED_FILENAMES.has(fileName)) continue;

    // Only allow known web extensions; skip files with no extension or disallowed ones
    if (!ext || !ALLOWED_EXTS.has(ext)) continue;

    out[origPath] = v.replace(/^\uFEFF/, "");
  }
...
// ═══════════════════════════════════════════════════════════════════════════
// REAL-TIME CODE INTELLIGENCE — per-file static analysis
// ═══════════════════════════════════════════════════════════════════════════

export interface FileIntelSignal {
  file: string;
  bytes: number;
  buttons: number;
  links: number;
...
/** Runs lightweight static analysis on one file as it finishes streaming.
 *  Pure CPU — no LLM calls. Returns a one-line intelligence headline. */
export function analyzeFileIntelligence(
  ...
  const buttons    = isHtml ? (content.match(/<button[\s>]|type=["']button["']/gi) ?? []).length : 0;
  const links      = isHtml ? (content.match(/<a\s+[^>]*href=/gi) ?? []).length : 0;
  const forms      = isHtml ? (content.match(/<form[\s>]/gi) ?? []).length : 0;
  const images     = isHtml ? (content.match(/<img[\s>]/gi) ?? []).length : 0;
  const apiCalls   = (content.match(/fetch\s*\(\s*['"`]\/api\//gi) ?? []).length;
  const selectors  = isCss  ? (content.match(/[.#][\w-]+\s*\{/g) ?? []).length : 0;
  const animations = isCss  ? (content.match(/@keyframes|animation:|transition:/gi) ?? []).length : 0;
  ...
  if (isHtml) {
    ...
    while ((m = linkRe.exec(content)) !== null) {
      ...
      if (!allFiles.has(target) && !allFiles.has(m[1])) brokenLinks++;
    }
  }
  const missingAlts = isHtml
    ? (content.match(/<img(?![^>]*\balt=)[^>]*>/gi) ?? []).length
    : 0;

  const parts: string[] = [];
  if (buttons > 0)    parts.push(`${buttons} btn${buttons > 1 ? "s" : ""}`);
  if (forms > 0)      parts.push(`${forms} form${forms > 1 ? "s" : ""}`);
  if (links > 0)      parts.push(`${links} link${links > 1 ? "s" : ""}`);
  if (selectors > 0)  parts.push(`${selectors} rules`);
  if (animations > 0) parts.push(`${animations} animation${animations > 1 ? "s" : ""}`);
  if (isJs && bytes > 1000) parts.push(`${(bytes / 1024).toFixed(1)} KB logic`);
  if (apiCalls > 0)   parts.push(`⚠ ${apiCalls} backend call${apiCalls > 1 ? "s" : ""}`);
  if (brokenLinks > 0) parts.push(`⚠ ${brokenLinks} broken link${brokenLinks > 1 ? "s" : ""}`);
  if (missingAlts > 0) parts.push(`${missingAlts} img${missingAlts > 1 ? "s" : ""} need alt`);

  const headline = parts.length > 0
    ? parts.slice(0, 4).join(" · ")
    : `${(bytes / 1024).toFixed(1)} KB — clean`;

  return { file: path, bytes, buttons, links, forms, images, apiCalls, brokenLinks, missingAlts, headline };
}

// ═══════════════════════════════════════════════════════════════════════════
// CONSCIOUSNESS PASS — post-build self-review and auto-patch
// ═══════════════════════════════════════════════════════════════════════════

export interface ConsciousnessReport {
  fixedLinks: number;
  fixedApiFetch: number;
  fixedMissingAlts: number;
  totalIssuesFound: number;
  totalIssuesFixed: number;
  findings: string[];
}

/** Reads all generated files, finds structural bugs, and patches them before
 *  the user ever sees the result. Pure CPU — no LLM calls. */
export async function consciousnessPass(
  files: SiteFiles,
): Promise<{ files: SiteFiles; report: ConsciousnessReport }> {
  const report: ConsciousnessReport = {
    fixedLinks: 0, fixedApiFetch: 0, fixedMissingAlts: 0,
    totalIssuesFound: 0, totalIssuesFixed: 0, findings: [],
  };

  const fixed: SiteFiles = { ...files };
  const allFilePaths = new Set(Object.keys(files));

  // ── 1. HTML files: broken links, missing alts, backend fetch calls ───────
  for (const [path, content] of Object.entries(fixed)) {
    if (!path.endsWith(".html")) continue;
    let c = content;

    // Fix broken internal href links
    const linkRe = /href="([^"#?]+\.html)"/g;
    let m: RegExpExecArray | null;
    while ((m = new RegExp(linkRe.source, linkRe.flags).exec(c)) !== null) {
      const target = m[1].replace(/^\.\//, "");
      if (!allFilePaths.has(target) && !allFilePaths.has(m[1])) {
        const fallback = allFilePaths.has("index.html")
          ? "index.html"
          : ([...allFilePaths].find(p => p.endsWith(".html")) ?? "#");
        report.fixedLinks++;
        report.findings.push(`Healed broken link ${target} → ${fallback} in ${path}`);
        c = c.replaceAll(`href="${m[1]}"`, `href="${fallback}"`);
      }
    }

    // Add missing alt attributes on <img> tags
    const imgNoAlt = /<img(?![^>]*\balt=)([^>]*)>/gi;
    const imgMatches = c.match(imgNoAlt) ?? [];
    if (imgMatches.length > 0) {
      report.fixedMissingAlts += imgMatches.length;
      c = c.replace(imgNoAlt, (tag) =>
        tag.endsWith("/>") ? tag.slice(0, -2) + ' alt="">' : tag.slice(0, -1) + ' alt="">',
      );
      report.findings.push(`Added alt="" to ${imgMatches.length} image${imgMatches.length > 1 ? "s" : ""} in ${path}`);
    }

    // Patch backend API fetch() calls → localStorage mock
    const backendFetchRe = /fetch\s*\(\s*(['"`])\/api\/[^'"` )]+\1[^)]*\)/g;
    const apiMatches = c.match(backendFetchRe) ?? [];
    if (apiMatches.length > 0) {
      report.fixedApiFetch += apiMatches.length;
      c = c.replace(
        backendFetchRe,
        `Promise.resolve({ok:true,json:()=>Promise.resolve(JSON.parse(localStorage.getItem('wf_data')||'[]'))})`,
      );
      report.findings.push(`Patched ${apiMatches.length} backend API call${apiMatches.length > 1 ? "s" : ""} in ${path} → localStorage`);
    }

    fixed[path] = c;
  }

  // ── 2. JS files: backend fetch calls ─────────────────────────────────────
  for (const [path, content] of Object.entries(fixed)) {
    if (!path.endsWith(".js")) continue;
    const backendFetchRe = /fetch\s*\(\s*(['"`])\/api\/[^'"` )]+\1[^)]*\)/g;
    const apiMatches = content.match(backendFetchRe) ?? [];
    if (apiMatches.length > 0) {
      report.fixedApiFetch += apiMatches.length;
  ...
  report.totalIssuesFound  = report.fixedLinks + report.fixedApiFetch + report.fixedMissingAlts;
  report.totalIssuesFixed  = report.totalIssuesFound;
  return { files: fixed, report };
}
